...
test_root="/home/deepzoom/TrafficInternVL/data-preparation/task1/data/test_part" # Store original test data
...
python3 extract_wts_test_frame_bbox_anno.py --root $test_root --save-folder $save_folder/frame_bbox_anno
python3 extract_bdd_test_frame_bbox_anno.py --root $test_root --save-folder $save_folder/frame_bbox_anno

for file in "$save_folder/frame_bbox_anno"/*test*; do
    python3 draw_bbox_on_frame.py --worker $num_worker --anno $file --scale $scale
done

python3 best_view_selection.py \
    --test-root $test_root \
    --save-path $save_folder/best_view_for_test.json \

python3 generate_test_frames.py \
